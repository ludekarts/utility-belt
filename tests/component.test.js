...
const { html, dynamicElement, createRepeater, fragment } = UtilityBelt;

describe("Dynamic Elements", () => {
  it("UTB Should have an Dynamic Elements API", () => {
    expect(html).to.be.a("function");
    expect(dynamicElement).to.be.a("function");
  });

  it("dynamicElement Should have '__setTerminateInterval' method", () => {
    expect(dynamicElement.__setTerminateInterval).to.be.a("function");
  });

  it("html`` Should create { inserts, markup } object", () => {
    const collection = html`<div class="${"hello"}"></div>`;
    expect(collection.markup).to.be.an("array");
    expect(collection.inserts).to.be.an("array");
    expect(collection.markup[0]).to.be.equal('<div class="');
    expect(collection.inserts[0]).to.be.equal("hello");
  });
  ...
  it("html(id)`` Should create { inserts, markup, id } object", () => {
    ...
    expect(collection.markup).to.be.an("array");
    expect(collection.inserts).to.be.an("array");
    expect(collection.id).to.be.an("string");
    expect(collection.markup[0]).to.be.equal('<div class="');
    expect(collection.inserts[0]).to.be.equal("hello");
    expect(collection.id).to.be.equal("myId");
  });
  ...
  it("Should create HTMLElement with 'update' method ", () => {
    const element = dynamicElement(() => html`<div class="hello"></div>`);
    expect(element.d.update).to.be.a("function");
  });

  it("Should create HTMLElement with 'refs' prop", () => {
    const element = dynamicElement(() => html`<div class="hello"></div>`);
    expect(element.d.refs).to.be.a("object");
  });

  it("Should create HTMLElement with reference to 'input' element in 'refs' prop", () => {
    const element = dynamicElement(
      () => html`<div class="hello"><input $ref="input" type="text" /></div>`
    );
    expect(element.d.refs.input).to.be.instanceof(HTMLElement);
  });
  ...
  it("Should return two inputs wrapped with div", () => {
    ...
    expect(element.lastChild.nodeName).to.be.equal("INPUT");
    expect(element.firstChild.nodeName).to.be.equal("INPUT");
  });
  ...
  it("Should embed HTMLElement into the template", () => {
    ...
    expect(selectedElement).to.be.equal(paragraph);
  });
  ...
  it("Should render element with default content", () => {
    ...
    expect(element.textContent).to.be.equal("hi");
  });
  ...
  it("Should update element's content with update() method", () => {
    ...
    expect(element.textContent).to.be.equal("hi");
    element.d.update("ho");
    expect(element.textContent).to.be.equal("ho");
  });
  ...
  it("Should replace text node with block node", () => {
    ...
    expect(element.innerHTML).to.be.equal("hello");
    element.d.update(paragraph);
    expect(element.innerHTML).to.be.equal("<p>paragraph</p>");
  });
  ...
  it("Should replace text node with array of elements", () => {
    ...
    const element = dynamicElement(
      (node) => html`<div id="ttoarroe">${node}</div>`,
      "hello"
    );
    expect(element.innerHTML).to.be.equal("hello");
    ...
    chai
      ...
      .to.be.equal("<span></span><strong></strong>");
  });
  ...
  it("Should replace node with array of elements", () => {
    ...
    const list = [nodeA, nodeB];
    const element = dynamicElement(
      (node) => html`<div id="ntoarroe">${node}</div>`,
      node
    );
    expect(element.innerHTML).to.be.equal("<p></p>");
    element.d.update(list);
    chai
      .expect(element.innerHTML)
      .to.be.equal("<span></span><strong></strong>");
  ...
  it("Should replace node with text node", () => {
    ...
    const text = "hi";
    const element = dynamicElement(
      (node) => html`<div id="txt">${node}</div>`,
      node
    );
    expect(element.innerHTML).to.be.equal("<p></p>");
    element.d.update(text);
    expect(element.innerHTML).to.be.equal("hi");
  });
  ...
  it("Should embed an array of HTMLElements into the template", () => {
    ...
    const list = [nodeA, nodeB];
    const element = dynamicElement(
      (list) => html`<div id="nodes">${list}</div>`,
      list
    );
    expect(element.firstElementChild).to.be.equal(nodeA);
    expect(element.lastElementChild).to.be.equal(nodeB);
  });
  ...
  it("Should replace array of HTMLElements with single node", () => {
    ...
    const list = [nodeA, nodeB];
    const element = dynamicElement(
      (content) => html`<div id="node">${content}</div>`,
      list
    );
    chai
      .expect(element.innerHTML)
  ...
  it("Should alow for embeding 'strings' and 'numbers' into template", () => {
    ...
    expect(elementA.textContent).to.be.equal("1");

    // Strings.
    const elementB = dynamicElement(() => html`<div id="str">${"hello"}</div>`);
    expect(elementB.textContent).to.be.equal("hello");
  });

  it("Should skip embeded 'functions' and 'objects' within template", () => {
    // Objects.
    const elementA = dynamicElement(() => html`<div id="obj">${{}}</div>`);
    expect(elementA.textContent).to.be.equal("");

    // Functions.
    const elementB = dynamicElement(
      () => html`<div id="fn">${() => false}</div>`
    );
    expect(elementB.textContent).to.be.equal("");
  });

  it("Should handle dynaimc and static attibutes", () => {
    const element = dynamicElement(
      () => html`<input data-index="${0}" type="text" value="${10}" />`
    );
    expect(element.getAttribute("value")).to.be.equal("10");
    expect(element.getAttribute("type")).to.be.equal("text");
    expect(element.getAttribute("data-index")).to.be.equal("0");
  });

  it("Should handle on-event attibutes", () => {
    let spy = chai.spy();
    const button = dynamicElement(
      () => html`<button onclick="${spy}">CLICK</button>`
    );
  ...
  it("Should not cache on-event handlers", () => {
    ...
    let render = ({ renderCount, counter }) => {
      ...
      return html`<button onclick="${updateCount}">
        RENDER ${renderCount}
      </button>`;
    };
    ...
    button.click();
    expect(spy).on.nth(1).be.called.with(0);
    expect(button.textContent).to.be.equal("RENDER 1");
    ...
    button.click();
    expect(spy).on.nth(2).be.called.with(1);
    expect(button.textContent).to.be.equal("RENDER 2");
  });
    ...
    const t = () =>
      dynamicElement(
        () =>
          html`<input
            type="text"
            value="${document.createTextNode("hello")}"
          />`
      );
    expect(t).to.throw();
  });
  ...
  it("Should update multiple attribute entries", () => {
    ...
    const container = dynamicElement(
      ({ a, b, c }) =>
        html`<div data-action="${a}" class="one ${b} three ${c}">Hello</div>`,
      { a, b, c }
    );

    expect(container.className).to.be.equal("one two three four");
    expect(container.dataset.action).to.be.equal("add");

    container.d.update({ a: a2, b: b2, c });
    expect(container.className).to.be.equal("one change three four");
    expect(container.dataset.action).to.be.equal("remove");
  });

  it("Should allow undefined attributes", () => {
    const element = dynamicElement(
      (type) => html`<input type="${type}" value="${10}" />`,
      "text"
    );
    ...
    expect(element.getAttribute("type")).to.be.equal("");

    element.d.update("number");
    expect(element.getAttribute("type")).to.be.equal("number");
  });
  ...
  it("Should handle boolean attributes", () => {
    ...
    expect(element.getAttribute("disabled")).to.be.equal(null);

    element.d.update(true);
    expect(element.getAttribute("disabled")).to.be.equal("disabled");
    expect(element.disabled).to.be.equal(true);
  ...
  it("Should update value prop when update attribute", () => {
    ...
    expect(element.getAttribute("value")).to.be.equal("ok");
    expect(element.value).to.be.equal("ok");
  });
  ...
  it("Should apply partial markup to main template", () => {
    ...
    expect(element.lastElementChild.textContent).to.be.equal("123");
  });
  ...
  it("Should update partial markup in main template", () => {
    ...
    expect(element.lastElementChild.textContent).to.be.equal("456");
  });

  it("Should render nestes partials markups", () => {
    const items = [1, 2, 3, 4, 5, 6];
    const sublist = (a, b, c) =>
      html`<span><strong>${a}</strong>${b}<em>${c}</em></span>`;
    const list = (items) =>
      html`<ul>
        <li>${items[0]}</li>
        <li>${items[1]}</li>
        <li>${items[2]}${sublist(items[3], items[4], items[5])}</li>
      </ul>`;
    const element = dynamicElement(
      (items) => html` <div>
        <strong>Hello</strong>
    ...
    expect(element.lastElementChild.textContent).to.be.equal("123456");
  });
  ...
  it("Should update nestes partials markups", () => {
    ...
    expect(element.lastElementChild.textContent).to.be.equal("1234ab");
  });
  ...
  it("Should replace nestes partials with DOM node", () => {
    const element = dynamicElement(
      (usePartial) =>
        html`<div>
          <strong>Hello</strong>${usePartial
            ? html`<span>Partial</span>`
            : "World"}
        </div>`,
      false
    );
    expect(element.textContent).to.be.equal("HelloWorld");
    element.d.update(true);
    expect(element.textContent).to.be.equal("HelloPartial");
    element.d.update(false);
    expect(element.textContent).to.be.equal("HelloWorld");
  });
  ...
  it("Should udate partial node in array with partials", () => {
    ...
    expect(element.lastElementChild.textContent).to.be.equal("1x3");
  });
  ...
  it("Should udate one partial with another", () => {
    ...
    const ToggleUi = (toggle) =>
      html`<div>
        <span>Hello</span>${toggle
          ? html`<span>World(${toggle + ""})</span>`
          : html`<span>Partial(${toggle + ""})</span>`}
      </div>`;
    const element = dynamicElement(ToggleUi, toggle);

    chai
      .expect(element.lastElementChild.textContent)
      .to.be.equal("Partial(false)");
    element.d.update(true);
    chai
      .expect(element.lastElementChild.textContent)
      .to.be.equal("World(true)");
  });

  it("Should not udate partials with same ID", () => {
    let renderPartial = () => html("PTL")`<span>Partial</span>`;

    expect(renderPartial()).to.have.have.property("id", "PTL");

    const PartialWrapper = () => html` <div>${renderPartial()}</div> `;

    const element = dynamicElement(PartialWrapper);
    expect(element.firstElementChild.textContent).to.be.equal("Partial");

    renderPartial = () => html("PTL")`<span>Changed</span>`;
    expect(renderPartial().markup[0]).to.contains("Changed");
    ...
    expect(renderPartial()).to.have.have.property("id", "PTL2");

    element.d.update();
    // Different ID - partial updated
    expect(element.firstElementChild.textContent).to.be.equal("Changed");
  });
  ...
  it("Should repeat elements", () => {
    ...
    const Items = (items) =>
      html`<ul $key="${(i) => i.id}" $items="${items}">
        ${ListItemUi}
      </ul>`;
    const list = dynamicElement(Items, items);

    expect(list.textContent).to.be.equal("Item 1Item 2Item 3Item 4Item 5");
  });
  ...
  it("Should reuse repeated elements", () => {
    ...
    expect(listA.children[1]).to.be.equal(item2Ref);
    expect(listA.children[1].textContent).to.be.equal("Changed");
  });
  ...
  it("Should re-render repeater when $items attribute changes", () => {
    ...
    expect(spy).to.have.been.called(9);
  });
  ...
  it("Should not re-render repeater when repeater fn changes", () => {
    ...
    const listElement = dynamicElement(List, items);
    expect(spyA).to.have.been.called(1);
  ...
  it("Should destroy repeater elements after given time", (done) => {
    ...
    setTimeout(() => {
      ...
      expect(listA.children[1]).to.not.be.equal(item2Ref);
